...
export class PollsList extends React.Component {
    ...
    onMakePollOfTheWeek(pollId) {
        ...
        for(let i=0; i < 10; i++) {
            PollsService.addVote({
                pollId: pollId,
                userId: UsersService.getLoggedUserId(),
                answerIndex: 0
            }).then(() => {
                ...
                <div className="list-item">
                        <div className="row">
                            ...
                            <div className="col-2 d-flex align-items-center justify-content-center text-right">
                                <span className="cursor-pointer mr-2"><FontAwesomeIcon icon={faPen} onClick={() => this.onEditPoll(poll.id)}/></span>
                                <span className="cursor-pointer mr-2"><FontAwesomeIcon icon={faTrashAlt} onClick={() => this.onDeletePoll(poll.id)} /></span>
                                <span className="cursor-pointer"><FontAwesomeIcon icon={faPoll} onClick={() => this.onMakePollOfTheWeek(poll.id)} /></span>
                            </div>
                        </div>
                    </div>
            );
